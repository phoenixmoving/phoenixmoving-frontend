...
export default async function PricingTable() {
  ...
  return (
    <Section className="bg-palette-background">
              ...
              <tbody className="divide-y divide-slate-200">
                {data.map((row, rowIndex) => (
                  <tr key={rowIndex} className="border-t border-slate-200">
                    <th
                      className="px-4 py-5 text-left text-sm font-normal text-slate-700"
                      scope="row"
                    >
                      {row.crew}
                    </th>
          ...
          <tbody className="divide-y divide-slate-200 border-t border-slate-200">
            {data.map((row) => {
              return (
                <tr key={row.crew}>
                  <th
                    className="px-6 py-8 text-left align-top text-lg font-semibold text-slate-900"
                    scope="row"
                  >
                    {row.crew}
                  </th>
                  {row.prices?.map((price, i) => (
                    <td
                      key={i}
                      className="h-full px-6 py-8 text-center align-top"
                    >
      ...
      <div className="mt-10 flex flex-col items-baseline justify-between gap-4 lg:flex-row">
        <p className="w-full text-sm font-light text-slate-900 lg:max-w-[550px]">
