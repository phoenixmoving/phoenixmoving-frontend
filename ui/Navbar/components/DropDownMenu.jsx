    ...
    <Popover className="relative">
      {({ open }) => (
        <>
          <Popover.Button
            className={clsx(
              "flex items-center font-medium transition-colors duration-150 ease-in-out",
              {
          ...
          <Popover.Panel className="absolute top-full -left-8 mt-3 grid w-80 gap-1 rounded-3xl border border-slate-200 bg-white p-2 shadow-lg">
            {items.map((item, i) => (
              <Popover.Button
                as={Link}
                key={i}
                ...
                className={clsx(
                  "group block rounded-2xl p-3 hover:bg-slate-100",
                  {
