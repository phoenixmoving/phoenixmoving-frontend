...
export default async function Pricing() {
  ...
  return (
    <Section className="bg-muted">
      ...
      <div className="mt-12 space-y-4 sm:mt-16 sm:grid sm:grid-cols-2 sm:gap-6 sm:space-y-0 lg:mx-auto lg:max-w-4xl xl:mx-0 xl:max-w-none xl:grid-cols-4">
        {tiers.map((tier) => (
          <div className="relative" key={tier.name}>
            {tier.isPopular && (
              <div className="absolute -top-[12px] left-0 right-0 mx-auto w-fit rounded-full bg-primary px-4 py-1 text-center text-xs font-medium text-white">
                MOST POPULAR
              </div>
            )}
            <div className="divide-y rounded-3xl bg-background shadow">
              <div className="p-6">
                <p className="text-lg font-semibold leading-6">{tier.name}</p>
                <p className="mt-4 text-sm text-muted-foreground">
                  {tier.description}
                </p>
                <p className="text-4xl font-semibold tracking-tight">
                  ${tier.priceMonthly}
                  <span className="text-sm text-muted-foreground ml-1 font-normal tracking-normal">
                    per hour
                  </span>
                </p>
        ...
        <div className="flex flex-col gap-4 sm:gap-6 justify-between">
          <div className="divide-y h-full rounded-3xl bg-background shadow">
            <div className="p-6">
      ...
      <div className="mt-12 rounded-3xl bg-primary shadow p-6 text-primary-foreground text-center flex flex-col">
